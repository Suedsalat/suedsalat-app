...
function upload_movie_tip_poster(array $file, array $allowedImageTypes, int $maxSizeBytes, ?string &$error): ?string
{
    ...
    $originalsDir = $movieTipsDir . '/originals';
    ...
    $publishedPath = $movieTipsDir . '/' . $filename;
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'add_review') {
    ...
    header('Location: ' . BASE_PATH . '/admin/movie-tips.php?edit=' . $reviewTipId . '#reviews');
    exit;
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'delete_review') {
    if (!verify_admin_password($pdo, $adminId, (string) ($_POST['confirm_password'] ?? ''))) {
        header('Location: ' . BASE_PATH . '/admin/movie-tips.php?edit=' . (int) $_POST['tip_id'] . '&delete_error=1#reviews');
        exit;
    }
    $stmt = $pdo->prepare('DELETE FROM tip_reviews WHERE id = :id AND tip_type = "movie_tip"');
    $stmt->execute([':id' => (int) $_POST['review_id']]);
    header('Location: ' . BASE_PATH . '/admin/movie-tips.php?edit=' . (int) $_POST['tip_id'] . '#reviews');
    exit;
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && in_array($_POST['action'] ?? '', ['move_up', 'move_down'], true)) {
    ...
    header('Location: ' . BASE_PATH . '/admin/movie-tips.php#tip-' . $moveId);
    exit;
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['title'])) {
    ...
    if ($title === '') {
        ...
        FcmSender::sendToAllDevices("Neuer Filmtipp: $title", "$pushAuthor hat einen neuen Filmtipp!");
...
if (isset($_GET['edit'])) {
    ...
    if ($editTip) {
        $stmt = $pdo->prepare('SELECT * FROM tip_reviews WHERE tip_type = "movie_tip" AND tip_id = :id ORDER BY created_at DESC');
