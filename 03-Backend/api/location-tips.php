...
$stmt = $pdo->query(
    ...
    'SELECT lt.id, lt.name, lt.location, lt.description, lt.link, lt.episode_guid, lt.episode_timestamp_seconds,
        ...
        (SELECT AVG(rating) FROM tip_reviews WHERE tip_type = "location_tip" AND tip_id = lt.id AND approved = 1 AND hidden_at IS NULL AND ' . Moderation::notReviewAccountSql("tip_reviews.listener_id") . ') AS avg_rating,
        (SELECT COUNT(*) FROM tip_reviews WHERE tip_type = "location_tip" AND tip_id = lt.id AND approved = 1 AND hidden_at IS NULL AND ' . Moderation::notReviewAccountSql("tip_reviews.listener_id") . ') AS review_count
