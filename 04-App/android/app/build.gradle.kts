...
// Release-Signing: Werte kommen aus android/key.properties (lokal, nie in Git,
// siehe .gitignore). Ohne diese Datei faellt der Release-Build auf den
// Debug-Key zurueck, damit `flutter run --release` auch ohne Keystore funktioniert.
...
android {
    namespace = "eu.suedsalat.suedsalat_app"
    ...
    buildTypes {
        release {
            signingConfig = if (keystorePropertiesFile.exists()) {
                ...
                // Fallback: Signing with the debug keys, so `flutter run --release` works
                // even without a local key.properties (e.g. auf einem anderen Rechner).
