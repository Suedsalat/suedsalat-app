...
/// Die App nutzt nur Libre Franklin aus assets/fonts/ (freie Lizenz, nichts wird nachgeladen).
...
void main() {
  ...
  final hinterlegt = {
    for (final m in RegExp(r'asset: (assets/fonts/LibreFranklin-\w+\.ttf)\s+weight: (\d+)').allMatches(pubspec))
      int.parse(m.group(2)!): m.group(1)!,
  };

  test('Schriftdateien liegen vor', () {
    expect(hinterlegt.keys, containsAll([400, 600]));
    for (final datei in hinterlegt.values) {
      expect(File(datei).existsSync(), isTrue, reason: datei);
    }
    expect(File('assets/fonts/LibreFranklin-OFL.txt').existsSync(), isTrue, reason: 'Lizenztext muss mit');
  });

  for (final (name, theme) in [('hell', AppTheme.light()), ('dunkel', AppTheme.dark())]) {
    test('Theme $name: nur Libre Franklin in hinterlegten Staerken', () {
      ...
      expect(t.titleLarge?.fontWeight, FontWeight.w600, reason: 'Ueberschriften halbfett wie im Logo');
    });
  }
